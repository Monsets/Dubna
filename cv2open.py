...
testing_file = args.get('test')
skip_count = args.get('skip')

if(testing_file):
    file = open(testing_file)
    ...
    for row in test_dataset:
        ...
        if (skip_count > 0):
            skip_count -= 1
            continue
        # Оптимизирую вычисления (стандартная функция в cv2)
        cv2.setUseOptimized(True)
        cv2.useOptimized()
        vs = cv2.VideoCapture('Train_data/' + row)
        print(row + ' in processing')

        # TODO: Make function with count a passenger
        ...
        while True:
            ...
            print(row + ' finish processing')
            print(passangers_count)
            print(test_dataset[row])
            print(test_dataset[row] - passangers_count)
